...
- Isolated-version probes (don't touch the user's daemon): `npm install --prefix .scratch-kimiXX @moonshot-ai/kimi-code@<ver>` (create the scratch `package.json` FIRST or npm pollutes the project manifest), then `KIMI_CODE_HOME=<Temp dir> node .scratch-kimiXX/.../main.mjs web --no-open --port 58997`; copy `config.toml` + `credentials/` into that home for a working model. Point `public/dev-token.json` at it for UI smoke tests, restore afterwards.
