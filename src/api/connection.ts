 ...
 *  token file written by `npm run dev:token` (gitignored, served by vite in
 *  dev only). The fallback must NEVER ship: in a production bundle a stale
 *  token file masks the real connection error with a wrong target. */
