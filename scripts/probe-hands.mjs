...
const dump = []
const created = await fetch(`${BASE}/api/v1/sessions`, { method: 'POST', headers: H, body: JSON.stringify({ title: 'Harness github probe', metadata: { cwd: 'C:/Users/user/Projects/KimiHarness' } }) }).then((r) => r.json())
...
ws.on('message', (m) => {
  ...
  if (f.type === 'ack' && f.id === 's1') fetch(`${BASE}/api/v1/sessions/${SID}/prompts`, { method: 'POST', headers: H, body: JSON.stringify({ content: [{ type: 'text', text: 'List your mcp__windows__* and mcp__playwright-personal__* tool names (names only, comma separated). If none, say MISSING.' }] }) })
})
