...
ws.on('message', (m) => {
  ...
  if (f.type === 'ack' && f.id === 's1') fetch(`${BASE}/api/v1/sessions/${SID}/prompts`, { method: 'POST', headers: H, body: JSON.stringify({ content: [{ type: 'text', text: 'Use the github MCP tools to list 3 of my repositories and tell me which account you authenticated as.' }] }) })
})
