...
ws.on('message', (m) => {
  ...
  if (f.type === 'ack' && f.id === 's1') fetch(`${BASE}/api/v1/sessions/${SID}/prompts`, { method: 'POST', headers: H, body: JSON.stringify({ content: [{ type: 'text', text: 'List the mcp__github__ tool names you have (just the names, comma separated). If none, say NO_GITHUB_TOOLS.' }] }) })
})
